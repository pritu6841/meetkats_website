...
const CreateTicketsPage = ({ user, onLogout }) => {
  ...
  useEffect(() => {
    const fetchEventDetails = async () => {
      try {
        ...
        const ticketTypesResponse = await ticketService.getEventTicketTypes(eventId, true);
  ...
  const validateTicketTypes = () => {
    ...
    const hasValidTicket = ticketTypes.some(ticket => ticket.name && (ticket.price || ticket.price === 0));
    ...
    for (let i = 0; i < ticketTypes.length; i++) {
      ...
      if (ticket.quantity && (isNaN(parseInt(ticket.quantity)) || parseInt(ticket.quantity) <= 0)) {
        return `Ticket type ${i + 1} requires a valid quantity (must be a positive number)`;
      }
      
      if (ticket.maxPerOrder && (isNaN(parseInt(ticket.maxPerOrder)) || parseInt(ticket.maxPerOrder) <= 0)) {
        return `Ticket type ${i + 1} requires a valid max per order (must be a positive number)`;
      }
      ...
      if (ticket.startSalesDate && ticket.endSalesDate) {
        ...
        if (start > end) {
          return `Ticket type ${i + 1} has an invalid date range (start date is after end date)`;
        }
      }
    }
    ...
    try {
      ...
      if (toast) {
        toast({
          ...
          description: `Successfully created ${validTicketTypes.length} ticket types for your event.`,
          status: "success",
          duration: 5000,
          isClosable: true,
        });
      ...
      setError(err.message || 'Failed to create ticket types. Please try again.');
      
      if (toast) {
        toast({
          title: "Error",
          description: err.message || 'Failed to create ticket types. Please try again.',
          status: "error",
          duration: 5000,
          isClosable: true,
        });
    ...
    return (
      <div className="flex h-screen">
        ...
        <div className="z-20 relative">
          <Sidebar user={user || authUser} onLogout={onLogout} />
        </div>
        
        {/* Main content */}
        <div className="flex-1 overflow-y-auto bg-orange-50">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div className="text-center bg-white rounded-lg shadow-sm p-8 border border-orange-100">
              <div className="inline-flex items-center justify-center w-16 h-16 bg-green-100 rounded-full mb-4">
                <CheckCircle className="w-8 h-8 text-green-600" />
              </div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Tickets Created Successfully!</h1>
              <p className="text-lg text-gray-600 mb-8">
                Your event tickets have been created and are ready for sales.
              </p>
              <div className="flex flex-col sm:flex-row justify-center space-y-3 sm:space-y-0 sm:space-x-4">
                ...
                <button 
                  className="inline-flex items-center px-4 py-2 border border-orange-300 text-sm font-medium rounded-md shadow-sm text-orange-700 bg-white hover:bg-orange-50 transition"
        ...
        <div className="bg-white border-b border-orange-200 shadow-sm sticky top-0 z-10">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex items-center justify-between">
              ...
              <h1 className="text-xl font-semibold text-gray-900">Create Tickets</h1>
              <div className="flex space-x-2">
                ...
                <button 
                  type="button" 
                  className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-orange-600 hover:bg-orange-700 transition disabled:bg-orange-300 disabled:cursor-not-allowed"
            ...
            <>
              ...
              <form id="create-tickets-form" onSubmit={handleSubmit}>
                {ticketTypes.map((ticket, index) => (
                  <div key={index} className="bg-white rounded-lg shadow-sm p-6 mb-6 border border-orange-100 hover:shadow-md transition">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <button 
                        ...
                        className="text-red-500 hover:text-red-700 transition disabled:text-red-300 disabled:cursor-not-allowed"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-3 py-2 border border-orange-200 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-500 shadow-sm"
                          required
                        />
                      </div>
                ...
                <div className="mb-6">
                  <button
                    ...
                    className="inline-flex items-center px-4 py-2 border border-orange-300 text-sm font-medium rounded-md text-orange-700 bg-white hover:bg-orange-50 shadow-sm transition"
                  >
                ...
                <div className="flex justify-end space-x-4">
                  <button
                    type="button"
                    onClick={handleSkip}
                    className="px-4 py-2 border border-orange-300 shadow-sm text-sm font-medium rounded-md text-orange-700 bg-white hover:bg-orange-50 transition"
                  >
                    Skip for Now
                  </button>
