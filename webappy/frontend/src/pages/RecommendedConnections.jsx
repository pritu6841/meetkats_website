...
const SuggestedUsersPage = () => {
  ...
  const fetchSuggestedUsers = async () => {
    ...
    try {
      ...
      console.log('Users with pending status:', filteredUsers.filter(u => u.isPending || u.connectionStatus === 'pending'));
  ...
  const handleConnect = async (userId) => {
    ...
    try {
      ...
      const pendingRequests = JSON.parse(localStorage.getItem('sessionPendingRequests') || '[]');
      console.log('Updated session pending requests in localStorage:', pendingRequests);
    } catch (error) {
  ...
  const handleMarkAllAsPending = () => {
    ...
    // Verify local storage was updated
    const pendingRequests = JSON.parse(localStorage.getItem('sessionPendingRequests') || '[]');
    console.log('After marking all, session pending requests:', pendingRequests);
  };
