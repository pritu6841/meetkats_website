...
const getCombinedPendingRequests = () => {
    ...
    const apiPendingRequests = JSON.parse(localStorage.getItem('pendingConnectionRequests') || '[]');
    
    // Get pending requests created during the current session
    const sessionPendingRequests = JSON.parse(localStorage.getItem('sessionPendingRequests') || '[]');
  ...
  const saveSessionPendingRequest = (userId) => {
    const sessionRequests = JSON.parse(localStorage.getItem('sessionPendingRequests') || '[]');
    
    if (!sessionRequests.includes(userId)) {
      sessionRequests.push(userId);
      localStorage.setItem('sessionPendingRequests', JSON.stringify(sessionRequests));
    }
  };
  
  // Helper function for extracting user ID from request ID
  const saveRequestUserMapping = (requestId, userId) => {
    const requestMap = JSON.parse(localStorage.getItem('requestUserMapping') || '{}');
  ...
  const networkService = {
    ...
    requestConnection: async (userId, message = '') => {
      try {
        ...
        console.log(`Sending connection request to user ${userId} with message: "${message}"`);
        ...
        const pendingRequests = JSON.parse(localStorage.getItem('pendingConnectionRequests') || '[]');
        if (!pendingRequests.includes(userId)) {
          pendingRequests.push(userId);
          localStorage.setItem('pendingConnectionRequests', JSON.stringify(pendingRequests));
        }
    ...
    syncPendingRequests: async () => {
      try {
        ...
        if (response.data && response.data.requests) {
          ...
          const pendingUserIds = requests.map(req => 
            req.recipient?._id || req.recipient
          );
          
          localStorage.setItem('pendingConnectionRequests', JSON.stringify(pendingUserIds));
        ...
        if (Array.isArray(response.data)) {
          const processedUsers = response.data.map(user => {
            ...
            if (user._id && pendingRequests.includes(user._id)) {
              console.log(`Marking user ${user._id} as pending (from combined requests)`);
              return {
                ...user,
    ...
    acceptConnection: async (requestId) => {
      try {
        ...
        if (userId) {
          ...
          const pendingRequests = JSON.parse(localStorage.getItem('pendingConnectionRequests') || '[]');
          const updatedAPIRequests = pendingRequests.filter(id => id !== userId);
          localStorage.setItem('pendingConnectionRequests', JSON.stringify(updatedAPIRequests));
          
          // Remove from session pending requests
          const sessionRequests = JSON.parse(localStorage.getItem('sessionPendingRequests') || '[]');
          const updatedSessionRequests = sessionRequests.filter(id => id !== userId);
          localStorage.setItem('sessionPendingRequests', JSON.stringify(updatedSessionRequests));
        }
    ...
    getConnectionRequests: async (status, direction) => {
      ...
      try {
        console.log(`Fetching connection requests with status: ${status}, direction: ${direction}`);
        ...
        if (direction === 'outgoing' && status === 'pending' && response.data && response.data.requests) {
  ...
  cancelConnectionRequest: async (requestId) => {
    try {
      // Input validation
      if (!requestId) {
        console.error('Invalid or missing request ID for canceling connection request');
        throw new Error('Invalid request ID');
      }
      
      const response = await api.delete(`/api/connections/requests/${requestId}`);
      ...
      if (userId) {
        ...
        localStorage.setItem('pendingConnectionRequests', JSON.stringify(updatedRequests));
      }
  ...
  updateConnectionNote: async (connectionId, note) => {
    try {
      ...
      const response = await api.put(`/api/connections/${connectionId}/note`, { note });
      return response.data;
    } catch (error) {
  ...
  updateLocationStatus: async (isVisible) => {
    try {
      const response = await api.put('/api/network/location-status', { isVisible });
      return response.data;
    } catch (error) {
  ...
  createMeetingRequest: async (meetingData) => {
    try {
      ...
      const response = await api.post('/api/network/meeting-request', meetingData);
      return response.data;
    } catch (error) {
  ...
  respondToMeetingRequest: async (meetingId, status, message, newMeetingTime) => {
    try {
      // Input validation
      if (!meetingId || !status) {
        console.error('Invalid meeting ID or status for responding to meeting request');
        throw new Error('Invalid meeting data');
      }
